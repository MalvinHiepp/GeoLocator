...
public class CityCommand extends Command {
    ...
    public void onCommand(User user, TextChannel channel, String[] args) throws IOException {
        switch (args.length) {
            case 0: {
                sendEmbed(Embed.getEmbed("**City**", "Please specify a ip", Color.RED));
                break;
            }
            case 1: {
                ...
                try {
                    sendEmbed(Embed.getEmbed("**City**", "City: " + jsonObject.getString("city"), Color.GREEN));
                } catch (Exception e) {
                    if (jsonObject.getString("status").equals("fail")) {
                        sendEmbed(Embed.getEmbed("**City**", "IP not found!",
                                Color.RED));
                    }
                }
            ...
            default: {
                sendEmbed(Embed.getEmbed("**City**", "Too many arguments", Color.RED));
            }
        }
    }
}
