...
public class IspCommand extends Command {

    @Override
    public void onCommand(User user, TextChannel channel, String[] args) throws IOException {
        switch (args.length) {
            case 0: {
                sendEmbed(Embed.getEmbed("**Isp**", "Please specify a ip", Color.RED));
                break;
            }
            case 1: {
                ...
                try {
                    sendEmbed(Embed.getEmbed("**Isp**", "\n Isp: " + jsonObject.getString("isp"), Color.GREEN));
                } catch (Exception e) {
                    if (jsonObject.getString("status").equals("fail")) {
                        sendEmbed(Embed.getEmbed("**Isp**", "IP not found!",
                                Color.RED));
                    }
                }
            ...
            default: {
                sendEmbed(Embed.getEmbed("**Isp**", "Too many arguments", Color.RED));
            }
        }
    }
}
