...
public class CountryCommand extends Command {
    ...
    public void onCommand(User user, TextChannel channel, String[] args) throws IOException {
        switch (args.length) {
            case 0: {
                sendEmbed(Embed.getEmbed("**Country**", "Please specify a ip", Color.RED));
                break;
            }
            case 1: {
                ...
                try {
                    sendEmbed(Embed.getEmbed("**Country**", "Country: " + jsonObject.getString("country"), Color.GREEN));
                } catch (Exception e) {
                    if (jsonObject.getString("status").equals("fail")) {
                        sendEmbed(Embed.getEmbed("**Country**", "IP not found!",
                                Color.RED));
                    }
                }
            ...
            default: {
                sendEmbed(Embed.getEmbed("**Country**", "Too many arguments", Color.RED));
            }
        }
    }
}
