...
public class AsnCommand extends Command {

    @Override
    public void onCommand(User user, TextChannel channel, String[] args) throws IOException {
        switch (args.length) {
            case 0: {
                sendEmbed(Embed.getEmbed("**Asn**", "Please specify a ip", Color.RED));
                break;
            }
            case 1: {
                ...
                try {
                    sendEmbed(Embed.getEmbed("**Asn**", "Asn: " + jsonObject.getString("as"), Color.GREEN));
                } catch (Exception e) {
                    if (jsonObject.getString("status").equals("fail")) {
                        sendEmbed(Embed.getEmbed("**Asn**", "IP not found!",
                                Color.RED));
                    }
                }
            ...
            default: {
                sendEmbed(Embed.getEmbed("**Asn**", "Too many arguments", Color.RED));
            }
        }
    }
}
